document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all(Array.from({ length: 32 }, loadCard)).then(cards => {
      cards.forEach((cardHTML, index) => {
          ...
          if (data) {
              ...
              card.querySelector(".card-number").textContent = `F${data.id}`;
              card.querySelector(".card-title").textContent = data.name_german;
              card.querySelector(".card-image").src = `images/${data.image}`;
              card.querySelector(".card-image").alt = data.name_german;
              card.querySelector(".card-trivia").textContent = data.trivia_german;

              // Statistiken setzen
              const stats = card.querySelectorAll(".stat-content");
              stats[0].textContent = `${data.max_weight} kg`; // Gewicht
              stats[1].textContent = `${data.max_length} cm`; // Länge
              stats[2].textContent = `${data.max_age} Jahre`; // Alter
              stats[3].textContent = `${data.top_speed} km/h`; // Geschwindigkeit
              stats[4].textContent = `${data.litter_size}`; // Wurfgröße
              stats[5].textContent = `${data.deaths}`; // Todesfälle
          }
